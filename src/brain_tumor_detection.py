...
def evaluate_model(model, test_loader, criterion, device):
    ...
    for i in range(len(class_names)):
        ...
        print(f"{class_names[i]}: {100 * class_correct / class_total:.2f}%")
...
def main():
    ...
    # Save model
    model_save_path = os.path.join('app', 'models', 'brain_tumor_model', 'brain_tumor_detection_model.pth')
    torch.save(model.state_dict(), model_save_path)
    print(f"\nModel saved as '{model_save_path}'")

    # Save weights
    weights_save_path = os.path.join('app', 'models', 'brain_tumor_model', 'model_weights.pth')
    torch.save(model.state_dict(), weights_save_path)
    print(f"Model weights saved as '{weights_save_path}'")
